...
local MOD_NAME = "BetterLandfillEverything"
local GUI_BUTTON = "ble_button"
...
function add_landfill(blueprint, player, is_shift_pressed, is_control_pressed, is_alt_pressed)
	local entities = blueprint.get_blueprint_entities()
	local old_tiles = blueprint.get_blueprint_tiles()
	local preferred_tile = settings.get_player_settings(player)[ble_defines.names.settings.preferred_tile].value
	local custom_tile = settings.get_player_settings(player)[ble_defines.names.settings.custom_tile].value

	-- Set up tile type to use, mod must be installed and chosen in settings.
	local landfill_tile = { name = ble_defines.tile_mapping[ble_defines.values.preferred_tile_values.default] }

	-- Basic checking if the selected tile is actually loaded
	if is_alt_pressed then
		if game.tile_prototypes[ble_defines.tile_mapping[preferred_tile]] then
			landfill_tile = { name = ble_defines.tile_mapping[preferred_tile] }
		else
			player.print("Better Landfill Everything could not find the selected: " ..
				preferred_tile .. " tile, is the mod loaded? Using default landfill")
		end
		if custom_tile then
			if game.tile_prototypes[custom_tile] then
				player.print("Better Landfill Everything is using custom tile: " .. custom_tile)
				landfill_tile = { name = custom_tile }
			else
				player.print("Better Landfill Everything could not find custom tile: " ..
					custom_tile .. " - using selected tile from settings menu if possible")
			end
		end
	end
	local tileIndex = 0
...
function get_ble_flow(player)

	local button_flow = mod_gui.get_button_flow(player)
	local flow = button_flow.ble_flow
	if not flow then
...
function add_top_button(player)
	...
	if player.gui.top.ble_flow then player.gui.top.ble_flow.destroy() end -- remove the old flow

	local flow = get_ble_flow(player)
	...
	flow.add {
		...
		tooltip = { "better_landfill_everything_tooltip" }
	}
end
...
script.on_event(defines.events.on_gui_click, function(event)
	...
	if event.element.name == GUI_BUTTON then
		if player.cursor_stack.valid_for_read then
			...
			if blueprint then
				local modified = add_landfill(blueprint, player, event.shift, event.control, event.alt)
...
script.on_configuration_changed(function(data)
	...
	if data.mod_changes[MOD_NAME] then
		...
		if oldVersion then
			...
			trace("Updating Better Landfill Everything from " .. tostring(oldVersion) .. " to " .. tostring(newVersion))
			--Reset top button
			for _, player in pairs(game.players) do
				add_top_button(player)
			end
